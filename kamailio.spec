...
Source:		http://kamailio.org/pub/kamailio/%{ver}/src/%{name}-%{ver}.tar.gz
Patch0:		%{name}-%{ver}-00-init-configs.patch
...
%{__mkdir} -p $RPM_BUILD_ROOT%{_sysconfdir}/systemd/system/multi-user.target.wants
%{__mkdir} -p $RPM_BUILD_ROOT%{_localstatedir}/run/%name
#install -m644 pkg/%name/centos/%{?centos}/%name.sysconfig \
#		$RPM_BUILD_ROOT/%{_sysconfdir}/sysconfig/%name
# Install init files
%{__install} -D -m644 pkg/%name/oracle/%{rhel}/%name.logrotated $RPM_BUILD_ROOT%{_sysconfdir}/logrotate.d/%name
%{__install} -D -m644 pkg/%name/oracle/%{rhel}/%name.rsyslogd $RPM_BUILD_ROOT%{_sysconfdir}/rsyslog.d/%name.conf
%{__install} -D -m644 pkg/%name/oracle/%{rhel}/%name.sysconfig $RPM_BUILD_ROOT%{_sysconfdir}/sysconfig/%name
%{__install} -D -m644 pkg/%name/oracle/%{rhel}/%name.service $RPM_BUILD_ROOT%{_unitdir}/%name.service
%{__ln_s} -f %{_unitdir}/%name.service $RPM_BUILD_ROOT%{_sysconfdir}/systemd/system/multi-user.target.wants/%name.service
...
%{_sysconfdir}/systemd/system/multi-user.target.wants
