...
static int ast_rtcp_write_rr(struct ast_rtp_instance *instance)
{
	...
	ast_ouraddrfor(&rtp->rtcp->them, &ourip);	
 	...
 	ast_verbose("\r\nSEND RR: SOURCE IP/PORT: [%s]:[%d]\n", ast_sockaddr_stringify_addr(&ourip), ast_sockaddr_port(&ourip));	
	ast_verbose("SEND RR: REMOTE IP/PORT: [%s]:[%d]\n", ast_sockaddr_stringify_addr(&rtp->rtcp->them), ast_sockaddr_port(&rtp->rtcp->them));	
	...
	res2 = send_hepv3 (rtp->rtcp->s, rcinfo, (unsigned int *)rtcpheader, len, 0);
	if (res2 < 0) {
...
static int ast_rtcp_write_sr(struct ast_rtp_instance *instance)
{
	...
	ast_ouraddrfor(&rtp->rtcp->them, &ourip);
 	...
 	ast_verbose("\r\nSEND SR: SOURCE IP/PORT: [%s]:[%d]\n", ast_sockaddr_stringify_addr(&ourip), ast_sockaddr_port(&ourip));	
	ast_verbose("SEND SR: REMOTE IP/PORT: [%s]:[%d]\n", ast_sockaddr_stringify_addr(&rtp->rtcp->them), ast_sockaddr_port(&rtp->rtcp->them));	
	...
	//ast_verbose("OURIP: %s\n", ast_sockaddr_stringify_addr(&ourip));
...
static struct ast_frame *ast_rtcp_read(struct ast_rtp_instance *instance)
{
	...
	/* HEP */
	gettimeofday(&now,NULL);
	chan = ast_rtp_instance_get_chan(instance);               
	
	ast_sockaddr_copy(&ourip, &rtp->rtcp->us);
	ast_ouraddrfor(&rtp->rtcp->them, &ourip);	

	ast_verbose("\r\nRECEIVE RTCP: SOURCE IP/PORT: [%s]:[%d]\n", ast_sockaddr_stringify_addr(&ourip), ast_sockaddr_port(&ourip));	
	ast_verbose("RECEIVE RTCP: REMOTE IP/PORT: [%s]:[%d]\n", ast_sockaddr_stringify_addr(&rtp->rtcp->them), ast_sockaddr_port(&rtp->rtcp->them));	
	...
	res2 = send_hepv3 (rtp->rtcp->s, rcinfo, rtcpheader, res, 0);
	if (res2 < 0) {
		ast_log(LOG_ERROR, "RTCP SR HEOP transmission error to %s, rtcp halted %s\n",
			ast_sockaddr_stringify(&rtp->rtcp->them),
			strerror(errno));
	...
	ast_verbose("CHANNEL [%s]\r\n", chan ? chan->uniqueid : "<null>");
	
	
	//if(!chan) ast_verbose("CHANNEL XPEH: [%s]\n", chan->name);
